...
router.post("/orders/:id/email-receipt", requireStaffAuth, async (req, res): Promise<void> => {
  ...
  const store = await getStoreSettings().catch(() => ({ storeName: "Restaurant", address: "", email: "", phone: "", timezone: "UTC", currency: "USD", taxRate: "0", id: 0 }));
  ...
  const tz = store.timezone ?? "America/Puerto_Rico";
  const receiptBody = `
    <tr><td style="padding:24px 32px 0">
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
          <td style="font-size:13px;color:#666">Order #</td>
          <td style="text-align:right;font-size:13px;color:#666">${new Date(order.createdAt).toLocaleString("en-US", { timeZone: tz })}</td>
        </tr>
        <tr>
          <td colspan="2" style="padding-top:4px">
            <span style="font-size:22px;font-weight:800;color:#1a1a1a">${order.confirmationCode}</span>
          </td>
        </tr>
      </table>
      <div style="margin-top:8px;font-size:14px;color:#555">Hi <strong>${order.customerName || "there"}</strong>, here is your receipt.</div>
    </td></tr>
    ...
    <tr><td style="padding:0 32px 8px">
      <table width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr><td style="font-size:18px;font-weight:800;color:#1a1a1a;padding:8px 0 4px;border-top:2px solid #1a1a1a">TOTAL</td><td style="text-align:right;font-size:18px;font-weight:800;color:#e05a00;border-top:2px solid #1a1a1a">${fmt(order.total)}</td></tr>
  ...
  const html = emailShell(receiptBody, { storeName: store.storeName, address: store.address ?? "", email: store.email ?? "" });
  ...
  try {
    ...
    await mailer.sendMail({
      from: smtpFrom,
      to: recipient,
      subject: `Your ${store.storeName} Receipt — Order #${order.confirmationCode}`,
      html,
    });
