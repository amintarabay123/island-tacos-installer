...
async function buildSystemPrompt(): Promise<string> {
  ...
  const profile = await getStoreSettings().catch(() => null);
  ...
  const storeName = profile?.storeName ?? "the restaurant";
  ...
  return `You are the friendly WhatsApp assistant for ${storeName}.
...
- For order status questions, direct them to: ${STORE_URL}/track or ask for their order code.
